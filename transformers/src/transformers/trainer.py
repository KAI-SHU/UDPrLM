...
class Trainer:
    ...
    def __init__(
        ...
        self.train_dataset,self.parsing_dataset = train_dataset
    ...
    def train(
        ...
        **kwargs,
        ...
        parsing_dataloader = self.get_train_dataloader(self.parsing_dataset).__iter__()
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if random.uniform(0,1)>0.5:
                    try:
                        parsing_inputs = parsing_dataloader.next()
                ...
                if (
                    ...
                    # Avoid unnecessary DDP synchronizat..
                        tr_loss += self.training_step(model, inputs, parsing_inputs)
                else:
                    tr_loss += self.training_step(model, inputs, parsing_inputs)
    ...
    def _prepare_parsing_inputs(self, parsing_inputs):
        for k,v in parsing_inputs.items():
            if isinstance(v, torch.Tensor):
                inputs[k] = v.to(self.args.device)
                
        return parsing_inputs

    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]],parsing_inputs) -> torch.Tensor:
        """
        ...
        parsing_inputs = self._prepare_parsing_inputs(parsing_inputs)

        if self.use_amp:
            with autocast():
                loss = self.compute_loss(model, inputs, parsing_inputs)
        else:
            loss = self.compute_loss(model, inputs, parsing_inputs)
    ...
    def compute_loss(self, model, inputs, parsing_inputs, return_outputs=False):
        """
        ...
        if self.label_smoother is not None and "labels" in inputs:
            ...
            labels = None
        outputs = model(**inputs, parsing_inputs)
