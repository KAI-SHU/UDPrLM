...
from supar.udlm import UDBertConfig, UDXLMRobertaConfig 
from supar.udlm import UDBertModel, UDXLMRobertaModel

MODEL_CLASSES = {
    "udbert": (UDBertConfig, UDBertModel),
    "udxlmr": (UDXLMRobertaConfig, UDXLMRobertaModel),
}
...
class TransformerEmbedding(nn.Module):
    ...
    def __init__(self, model, model_type, n_layers, fusion_ud=False, n_out=0, stride=256, pooling='mean', pad_index=0, dropout=0, requires_grad=False):
        super().__init__()
        ...
        if model_type in MODEL_CLASSES:
            ...
            self.bert = model_class.from_pretrained(model, config=config_class.from_pretrained(model, output_hidden_states=True))
            self.bert.use_ud_repr = fusion_ud
            print('num_labels:',self.bert.config.num_labels)
            print('use_ud_repr:',self.bert.use_ud_repr)
            print('num_ud_labels:', self.bert.config.num_ud_labels)
